local repo = 'https://raw.githubusercontent.com/scripade/sh/refs/heads/main/'


local Library = loadstring(game:HttpGet(repo .. 'main.txt'))()


Library:Notify('Ready To Clap Niggers?')
wait(1)


local ThemeManager = loadstring(game:HttpGet(repo .. 'Gaay/ThemeManager.txt'))()
local SaveManager = loadstring(game:HttpGet(repo .. 'Gaay/SaveManager.lua.txt'))()
...
RightGroupBox:AddDropdown('AutoPredMode', {
    ...
    Default = getgenv().Psalms.AutoPredMode == 'Blatant' and 2 or 1, -- Default to PingBased
...
MacroGroup:AddButton('Macro', function()
    if getgenv().Psalms.Macro == "OFF" then
        getgenv().Psalms.Macro = "ON"
        getgenv().Speed = 0.01 -- Lower value = faster macro speed
        loadstring(game:HttpGet("https://raw.githubusercontent.com/scripade/Fly-gui/refs/heads/main/SPEED"))()
        Library:Notify('Macro Enabled')
    else
        getgenv().Psalms.Macro = "OFF"
...
local AutoArmorGroup = Tabs.Misc:AddLeftGroupbox('Auto Armor')

AutoArmorGroup:AddToggle('AutoArmorEnabled', {
    Text = 'Enable Auto Armor (Da Strike)',
    Default = false,
    Callback = function(Value)
...
game:GetService("RunService").Heartbeat:Connect(function()
    if getgenv().AutoArmor then
        if not IsArmorEquipped() then  -- Only buy armor if it's missing
            BuyArmor()
            wait(1.5)  -- Wait for armor to appear in the backpack
            EquipArmor()
        end
    end
end)
...
--fly
local FlyGroup = Tabs.Misc:AddLeftGroupbox('Fly')  

FlyGroup:AddButton('Fly', function()
    loadstring(game:HttpGet("https://rawscripts.net/raw/Universal-Script-Mobile-Fly-20327"))()
    Library:Notify('Fly script loaded')
end)
...
ThemeManager:SetFolder('GEEK/VRTheme')
SaveManager:SetFolder('GEEK/VRCFG')
...
local function initializeToggleButton(screenGui)
    ...
    button.TextColor3 = Color3.new(255, 5, 85)
...
local function toggleLock()
    if enabled then
        ImageButton.Image = "rbxassetid://120805421574890"
        enabled = false
        Plr = nil
        destroyTracer()  
...
local Blatant = {
    ...
    {260, 0.1984736},
    {270, 0.138973450},
    {280, 0.3248543265473753},
    {290, 0.3248543265473753},
    {300, 0.3248543265473753},
}
